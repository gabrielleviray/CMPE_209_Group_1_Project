...
def kddReducetoFour(df):

    kddFourClasses = df.copy()
    ...
    attackType = kddFourClasses.pop('attack_type')
    kddFourClasses['attack_type'] = attackType
    return kddFourClasses
...
def kddDfEncode(kdd):
    ...
    kddFourClassesEncoded = kddReducetoFour(kdd)
    kddFourClassesEncoded = pd.get_dummies(kddFourClassesEncoded, columns=['protocol_type','flag','service'])
...
def kddMutualInfo(df):
    ...
    end_time = time.time()
    print("Mutual Info time: ",end_time-start_time)
